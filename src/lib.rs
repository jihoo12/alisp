...
impl Evaluator {
    ...
    fn builtin_re_test(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 2 {
    ...
    fn builtin_re_find(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 2 {
    ...
    fn builtin_re_find_all(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 2 {
    ...
    fn builtin_re_match(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 2 {
    ...
    fn builtin_re_replace(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 3 {
    ...
    fn builtin_re_replace_all(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 3 {
    ...
    fn builtin_re_split(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 2 {
    ...
    fn builtin_re_scan(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 2 {
...
impl RProg {
    ...
    fn is_match(&self, input: &str) -> bool {
        let chars: Vec<char> = input.chars().collect();
        let start = 0;
        match_seq(&self.ast, &chars, start).is_some()
    }
...
fn match_node(node: &RNode, input: &[char], pos: usize) -> Option<usize> {
    match node {
        ...
        // Quantifiers: wrap in single-element seq so match_nodes handles backtracking
