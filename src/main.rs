...
fn json_parse_str(s: &str) -> Result<Expr, String> {
    let mut pos = 0;
...
fn json_stringify(e: &Expr, compact: bool) -> String {
...
impl Evaluator {
    ...
    fn set_global(&mut self, name: String, val: Expr) {
    ...
    fn num_args(&self, args: &[Expr]) -> Result<Vec<f64>, String> {
        args.iter()
            .map(|a| match a {
                Expr::Num(n) => Ok(*n),
                _ => Err(format!("Expected number, got {}", expr_to_string(a))),
            })
            .collect()
    }
    ...
    fn builtin_add(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() == 1 {
            return self.num_args(args).map(|v| Expr::Num(v[0]));
        }
    ...
    fn builtin_lt(&mut self, args: &[Expr]) -> Result<Expr, String> {
        ...
        let a = self.to_num(&args[0])?;
        let b = self.to_num(&args[1])?;
        Ok(Expr::Bool(a < b))
    }
    ...
    fn to_num(&self, e: &Expr) -> Result<f64, String> {
        match e {
            Expr::Num(n) => Ok(*n),
            Expr::Str(s) => s.parse::<f64>().map_err(|_| format!("Cannot convert '{}' to number", s)),
    ...
    fn builtin_int(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.is_empty() {
            return Err("int requires an argument".into());
        }
        match &args[0] {
    ...
    fn builtin_float(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.is_empty() {
            return Err("float requires an argument".into());
        }
        match &args[0] {
    ...
    fn builtin_is_number(&mut self, args: &[Expr]) -> Result<Expr, String> {
        ...
        Ok(Expr::Bool(matches!(&args[0], Expr::Num(_))))
    }
    ...
    fn builtin_is_list(&mut self, args: &[Expr]) -> Result<Expr, String> {
        ...
        Ok(Expr::Bool(matches!(&args[0], Expr::List(_))))
    }
    ...
    fn builtin_is_nil(&mut self, args: &[Expr]) -> Result<Expr, String> {
        ...
        Ok(Expr::Bool(matches!(&args[0], Expr::Nil)))
    }
    ...
    fn builtin_eprint(&mut self, args: &[Expr], newline: bool) -> Result<Expr, String> {
        let s: String = args.iter().map(expr_to_string).collect();
        if newline {
            eprintln!("{}", s);
        } else {
            eprint!("{}", s);
    ...
    fn builtin_json_parse(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.is_empty() {
            return Err("json-parse requires a string".into());
        }
        let s = expr_to_string(&args[0]);
        json_parse_str(&s)
    }
    ...
    fn builtin_json_stringify(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.is_empty() {
            return Err("json-stringify requires a value".into());
        }
    ...
    fn builtin_json_get(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 2 {
            return Err("(json-get obj key)".into());
        }
        let key = expr_to_string(&args[1]);
        match &args[0] {
            Expr::List(v) => {
    ...
    fn builtin_json_set(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.len() < 3 {
            return Err("(json-set obj key value)".into());
        }
        let key = expr_to_string(&args[1]);
        let val = args[2].clone();
        match &args[0] {
    ...
    fn builtin_sleep(&mut self, args: &[Expr]) -> Result<Expr, String> {
        if args.is_empty() {
            return Err("sleep requires duration in seconds".into());
        }
...
fn glob_simple(pattern: &str) -> Vec<String> {
    // Simple glob: handle * in the last path component
...
fn main() {
    ...
    if args.is_empty() {
        ...
        loop {
            ...
            match eval_input(&mut eval, &full_input) {
                Ok(Some(result)) => {
        ...
        if let Err(e) = eval_input_str(&mut eval, &code) {
            eprintln!("Error: {}", e);
            std::process::exit(1);
        }
    } else {
...
fn eval_input(eval: &mut Evaluator, input: &str) -> Result<Option<Expr>, String> {
...
fn eval_input_str(eval: &mut Evaluator, input: &str) -> Result<(), String> {
    let _ = eval_input(eval, input)?;
    Ok(())
}
